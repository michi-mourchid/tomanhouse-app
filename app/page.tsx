...
export default function TomanWaitlist() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    console.log({ ...formData, gender: selectedGender, event: selectedDate })
  }
          ...
          <div className="space-y-2">
            {events.map((event) => (
              <Button
                key={event.id}
                type="button"
                variant={selectedDate === event.id ? "default" : "outline"}
                onClick={() => setSelectedDate(event.id)}
                className={`w-full justify-start ${selectedDate === event.id
                  ? "bg-blue-600 hover:bg-blue-700 text-white"
                  : "bg-white/10 border-white/20 text-white hover:bg-white/20"
                  }`}
              >
